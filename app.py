...
def load_data():
    """
    Load and validate CSV files containing truck fill data, using chunking to reduce memory usage.
    ...
    required_columns = ['Tonnage', 'Truck Factor', 'Shovel', 'Material', 'Time Full', 'Assigned Dump']
    data = []
    ...
    for month in months:
        ...
        try:
            ...
            if file_size > 500:
                st.warning(f"File {month}.csv is large ({file_size:.2f} MB). Consider splitting it.")
            
            # Read CSV in chunks with specific dtypes to reduce memory
            chunk_list = []
            for chunk in pd.read_csv(
                file_path,
                usecols=[col for col in required_columns if col in pd.read_csv(file_path, nrows=1).columns],
                chunksize=chunksize,
                dtype={'Tonnage': 'float32', 'Truck Factor': 'float32', 'Shovel': 'category', 'Material': 'category', 'Assigned Dump': 'category'},
                parse_dates=['Time Full']
            ):
                if all(col in chunk.columns for col in ['Tonnage', 'Truck Factor', 'Shovel']):
                    chunk = chunk[(chunk['Truck Factor'] > 0) & (chunk['Tonnage'] > 0)]
                    if not chunk.empty:
                        chunk_list.append(chunk)
                else:
                    st.warning(f"Missing required columns in {month}.csv chunk. Skipping chunk...")
            
            if chunk_list:
                month_data = pd.concat(chunk_list, ignore_index=True)
            ...
            st.error(f"MemoryError while loading {month}.csv. Try reducing chunksize or file size.")
            return []
