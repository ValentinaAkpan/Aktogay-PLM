...
def process_loaded_data(data):
    ...
    all_data['Year'] = pd.to_datetime(all_data['Time Full']).dt.year  # Extract the year
    return all_data
...
def load_truck_fill_data(data):
    ...
    result_df = pd.DataFrame.from_records(all_months_data)
...
def main():
    ...
    shovel_fill_data = load_shovel_fill_data(data, selected_shovel)
    fig = plot_distribution(shovel_fill_data, selected_shovel, selected_mean, selected_std)
    ...
    results_df = load_truck_fill_data(data)
